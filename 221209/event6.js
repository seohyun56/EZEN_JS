...
dts.forEach(dt => {
    ...
    dt.style.color = '#fff';
    dt.style.backgroundColor = 'royalblue';
});

const event1 = document.querySelector('ol > li:nth-child(1)');
...
event6.addEventListener('mouseover', () => event6.style.background = 'lightgreen');
event7.addEventListener('mouseout', () => event7.style.background = 'lightpink');
// 하위 노드에 영향을 주지 않음
event8.addEventListener('mouseenter', () => event8.style.border = '1px solid #000');
event9.addEventListener('mouseleave', () => event9.style.border = '1px solid #424242');
